...
type Props = {
  ...
  onStopConfirm?: (loc: LatLng, address: string, continueAdding: boolean) => void;
...
export default function CustomerMapPlanner({
  pickup, pickupAddress, dropoff, dropoffAddress, stops = [], stopRequestKey = 0,
  onStopConfirm, onStopRemove, onStopsClear,
  onPickupChange, onDropoffChange, onPickupPlaceSelect, onDropoffPlaceSelect, children,
}: Props) {
  ...
  const usingPrimaryMarker = useRef<"pickup" | "dropoff" | null>(null);
  ...
  const activeLabel = stage === "pickup" ? "نقطة الانطلاق" : stage === "dropoff" ? "نقطة الوصول" : `نقطة التوقف ${Math.min(stops.length + 1, 3)}`;
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(dismissGuide, 15000);
    return () => window.clearTimeout(t);
  ...
  const openPickup = useCallback(() => {
    ...
    usingPrimaryMarker.current = null;
    dismissGuide();
    setStage("pickup");
  ...
  const recenter = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(pos => {
      mapRef.current?.flyTo({ center: [pos.coords.longitude, pos.coords.latitude], zoom: 16, offset: [0, 70], duration: 450 });
  ...
  useEffect(() => {
    ...
    map.on("load", () => { map.resize(); window.setTimeout(() => map.resize(), 250); });
  ...
  useEffect(() => {
    ...
    if (usingPrimaryMarker.current === stage && (stage === "pickup" || stage === "dropoff")) {
      draftMarker.current?.remove();
      draftMarker.current = null;
      return;
    }
    draftMarker.current?.remove();
    const kind: PinKind = stage === "pickup" ? "pickup" : stage === "dropoff" ? "dropoff" : stage === "stop" ? "stop" : "draft";
  ...
  useEffect(() => {
    ...
    if (!navigator.geolocation) { setLocating(false); setStage("pickup"); return; }
    ...
    }, () => { setLocating(false); setStage("pickup"); }, { enableHighAccuracy: true, timeout: 10000, maximumAge: 30000 });
  }, []);

  useEffect(() => {
    if (stopRequestKey === lastStopRequest.current) return;
    lastStopRequest.current = stopRequestKey;
    if (!pickup || !dropoff || stops.length >= 3) return;
    usingPrimaryMarker.current = null;
    dismissGuide();
    setStage("stop");
    setDraft(null);
  ...
  }, [stopRequestKey, pickup, dropoff, stops.length, dismissGuide]);
  ...
  const confirmStop = (continueAdding: boolean) => {
    if (!draft || stage !== "stop") return;
    const current = draft;
    onStopConfirm?.(current.loc, current.address, continueAdding);
    setDraft(null);
    setQuery("");
    setSearchOpen(false);
    usingPrimaryMarker.current = null;
    if (continueAdding && stops.length < 2) {
      setStage("stop");
      setRouteCardSmall(true);
    } else {
      setStage("done");
  ...
  const confirmDraft = async () => {
    ...
    usingPrimaryMarker.current = null;
    if (stage === "pickup") {
      onPickupChange(current.loc, current.address);
      onPickupPlaceSelect?.(current.placeId);
      setStage("dropoff");
      setQuery("");
      setSearchOpen(false);
      const map = mapRef.current;
      if (map) {
        ...
        const p = map.unproject([canvas.clientWidth * 0.5, canvas.clientHeight * 0.62]);
        const loc = { lat: p.lat, lng: p.lng };
        setDraft({ loc, address: await reverseGeocode(loc), placeId: null });
        map.easeTo({ center: [current.loc.lng, current.loc.lat], zoom: Math.max(map.getZoom(), 15), offset: [0, 100], duration: 250 });
      } else setDraft(null);
    } else if (stage === "dropoff") {
      onDropoffChange(current.loc, current.address);
      onDropoffPlaceSelect?.(current.placeId);
      setStage("done");
      setDraft(null);
      setQuery("");
      setSearchOpen(false);
  ...
  const moveRouteDrag = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    setRouteCardY(Math.max(74, Math.min(window.innerHeight * .55, routeDrag.current.top + e.clientY - routeDrag.current.y)));
  };

  const recenterTop = Math.min(
    routeCardY + (routeCardSmall ? 72 : 164) + 14,
    Math.max(180, (typeof window !== "undefined" ? window.innerHeight : 800) - 330),
  );

  return <div className="cmpShell" dir="rtl">
    <div ref={mapEl} className="cmpMap" />

    <button type="button" className="cmpRecenterBtn" onClick={recenter} aria-label="العودة إلى موقعي الحالي" title="موقعي الحالي" style={{ top: recenterTop }}>
      <svg viewBox="0 0 48 48" aria-hidden="true"><circle cx="24" cy="24" r="13"/><circle className="center" cx="24" cy="24" r="3.5"/><path d="M24 3v8M24 37v8M3 24h8M37 24h8"/></svg>
    </button>

    <div className={`cmpTopCard ${routeCardSmall ? "small" : ""} ${guideVisible ? "guided" : ""}`} style={{ top: routeCardY }} onClickCapture={dismissGuide}>
      <div className="cmpFloatHandle" onPointerDown={startRouteDrag} onPointerMove={moveRouteDrag} onPointerUp={() => { routeDrag.current = null; }} onPointerCancel={() => { routeDrag.current = null; }}>
        <span/><button type="button" onClick={() => setRouteCardSmall(v => !v)}>{routeCardSmall ? "□" : "—"}</button>
      </div>
      {!routeCardSmall ? <>
        <div className="cmpPointRow pickup"><button type="button" className="cmpReselect" onClick={openPickup}>↻</button><button type="button" className="cmpPoint" onClick={openPickup}><span className="cmpDot green"/><span className="cmpAddressText"><small>من</small><b title={pickupAddress}>{pickupAddress || (locating ? "جارٍ تحديد موقعك..." : "حدد نقطة الانطلاق")}</b></span></button></div>
      ...
      </> : <button type="button" className="cmpMiniRoute" onClick={() => setRouteCardSmall(false)}><span className="cmpDot green"/><b>من</b><span>←</span><span className="cmpDot blue"/><b>إلى</b></button>}
    </div>
    ...
    {stage === "stop" && !searchOpen && !draft && <div className="cmpStopPrompt cmpStopSession">
      <div className="cmpStopPromptText"><b>حدد نقطة التوقف {Math.min(stops.length + 1, 3)}</b><span>اضغط على الخريطة أو ابحث عن المكان</span></div>
      <button type="button" className="cmpStopSearch" onClick={() => setSearchOpen(true)}>بحث</button>
      {stops.length > 0 && <div className="cmpStopSessionList">{stops.map((s, i) => <span key={`${i}-${s.addr}`}><b>محطة {i + 1}</b><button type="button" onClick={() => onStopRemove?.(i)} aria-label={`حذف محطة ${i + 1}`}>×</button></span>)}<button type="button" className="cmpClearStops" onClick={onStopsClear}>حذف الكل</button></div>}
    </div>}

    {draft && !searchOpen && <div className={`cmpConfirmCard ${stage === "stop" ? "stop" : ""}`}>
      <div><small>{activeLabel}</small><b>{draft.address}</b><span>يمكنك سحب الدبوس لضبط المكان بدقة</span></div>
      {stage === "stop" ? <div className="cmpStopConfirmActions">
        <button type="button" className="primary" onClick={() => confirmStop(false)}>تأكيد</button>
        {stops.length < 2 && <button type="button" className="secondary" onClick={() => confirmStop(true)}>نقطة أخرى</button>}
        <button type="button" className="cancel" onClick={() => setDraft(null)} aria-label="إلغاء النقطة الحالية">×</button>
      </div> : <button type="button" onClick={() => void confirmDraft()}>{stage === "pickup" ? "تأكيد نقطة الانطلاق" : "تأكيد نقطة الوصول"}</button>}
    </div>}

    {stage === "stop" && stops.length > 0 && <div className="cmpStopChips" aria-label="نقاط التوقف المؤكدة">{stops.map((s, i) => <span key={`chip-${i}-${s.addr}`}><b>{i + 1}</b><button type="button" onClick={() => onStopRemove?.(i)} aria-label={`حذف نقطة التوقف ${i + 1}`}>×</button></span>)}<button type="button" className="clear" onClick={onStopsClear}>× الكل</button></div>}

    {stage === "done" && children && <div className="cmpTripSheet">{children}</div>}

    {searchOpen && <section className="cmpSearchSheet" aria-label={`بحث ${activeLabel}`}>
      <div className="cmpGrab"/>
      <header><button type="button" onClick={() => setSearchOpen(false)}>×</button><div><small>{activeLabel}</small><h2>{stage === "pickup" ? "اختر نقطة الانطلاق" : stage === "dropoff" ? "أدخل وجهتك" : `أضف نقطة توقف ${Math.min(stops.length + 1, 3)}`}</h2></div></header>
      <div className="cmpSearchBox">⌕<input autoFocus value={query} onChange={e => setQuery(e.target.value)} placeholder={stage === "pickup" ? "ابحث عن مكان الانطلاق..." : stage === "dropoff" ? "ابحث عن مكان الوصول..." : "ابحث عن نقطة التوقف..."}/>{query && <button type="button" onClick={() => setQuery("")}>×</button>}</div>
      <div className="cmpResults">{searching && <p className="cmpStatus">جارٍ البحث في الأماكن المحفوظة والخريطة...</p>}{!searching && query.trim() && results.length === 0 && <p className="cmpStatus">لا توجد نتائج مطابقة — يمكنك اختيار النقطة مباشرة من الخريطة.</p>}{results.map(r => <button type="button" key={r.key} className="cmpResult" onClick={() => choose(r)}><span className={`cmpResultIcon ${r.source}`}>⌖</span><span><b><Highlight text={r.name} query={query}/></b>{r.context && <small><Highlight text={r.context} query={query}/></small>}<em>{r.source === "saved" ? "مكان محفوظ في كابتن بنها" : "نتيجة من الخريطة"}</em></span></button>)}</div>
    </section>}
  </div>;
}
