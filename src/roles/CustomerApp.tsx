...
export default function CustomerApp() {
  ...
  useEffect(() => {
    ...
    drivingRouteKm(points).then(d => {
      if (cancelled) return;
      setDistance(d); const k = guessKind(d); setKind(k);
  ...
  useEffect(() => {
    ...
    setFare(roundFareDownTo5(Math.max(Math.round(distance * ppk * 100) / 100, settings.min_fare)));
  ...
  const pickFixedRoute = (r: FixedRoute, reversed: boolean) => {
    ...
    setPickupPlaceId(from.id); setDropoffPlaceId(to.id); setStops([]); setPickedRouteId(r.id + (reversed ? "-r" : ""));
  };

  const clearRoute = () => { setPickup(null); setPickupAddr(""); setDropoff(null); setDropoffAddr(""); setPickupPlaceId(null); setDropoffPlaceId(null); setFixedPrice(null); setStops([]); setPickedRouteId(null); setDistance(null); setFare(null); setRouteError(""); };
  ...
  const pickFavorite = (t: { pickup: LatLng; pickupAddr: string; dropoff: LatLng; dropoffAddr: string }) => {
    ...
    setPickupPlaceId(null); setDropoffPlaceId(null); setFixedPrice(null); setStops([]); setPickedRouteId(null); setTab("home");
  };

  const bookingDetails = (
    <div className="cmpBookingDetails">
      <div className="cmpBookingHead"><b>تفاصيل الرحلة</b><button type="button" onClick={clearRoute}>بدء اختيار جديد</button></div>
      ...
      {stops.map((s, i) => <div className="stopRow" key={i}><MapPicker label={`نقطة توقف ${i + 1}`} color="amber" value={s.loc} address={s.addr} onChange={(loc, addr) => setStops(arr => arr.map((x, j) => j === i ? { loc, addr } : x))}/><button type="button" className="stopRemove" onClick={() => setStops(arr => arr.filter((_, j) => j !== i))}>✕ إزالة</button></div>)}
      {stops.length < MAX_STOPS && <button type="button" className="addStopBtn" onClick={() => { setStops(arr => [...arr, { loc: null, addr: "" }]); setPickedRouteId(null); }}>＋ إضافة نقطة توقف ({stops.length}/{MAX_STOPS})</button>}
      <div className="field"><label>نوع الرحلة</label><div className="segmented"><button className={kind === "in_city" ? "on" : ""} onClick={() => setKind("in_city")} type="button">داخل المدينة</button><button className={kind === "intercity" ? "on" : ""} onClick={() => setKind("intercity")} type="button">بين المدن</button></div></div>
      ...
      <button className="cta" onClick={requestTrip} disabled={busy || routeLoading || !distance || !!routeError}>{busy ? "جارٍ الإرسال..." : routeLoading ? "جارٍ حساب الطريق..." : "اطلب رحلة"}</button>
    </div>
  );

  return <div className="roleShell" dir="rtl"><TopBar title="كابتن بنها — العميل"/><main className="roleMain"><div className="lpTabs"><button className={tab === "home" ? "on" : ""} onClick={() => setTab("home")}>الرئيسية</button><button className={tab === "trips" ? "on" : ""} onClick={() => setTab("trips")}>رحلاتي</button><button className={tab === "ratings" ? "on" : ""} onClick={() => setTab("ratings")}>تقييماتي</button></div>
    {tab === "trips" && <MyTrips isCustomer onPickFavorite={pickFavorite}/>} {tab === "ratings" && <MyRatings/>}
    {tab === "home" && (activeTrip?.status === "pending" ? <CustomerOffers tripId={activeTrip.id} onAccepted={() => void checkActive()} onCancel={() => { setActiveTrip(null); void checkActive(); }}/> : activeTrip ? <ActiveTrip onDone={() => { setActiveTrip(null); void checkActive(); }}/> : <CustomerMapPlanner pickup={pickup} pickupAddress={pickupAddr} dropoff={dropoff} dropoffAddress={dropoffAddr} onPickupChange={(loc, addr) => { setPickup(loc); setPickupAddr(addr); setPickedRouteId(null); }} onDropoffChange={(loc, addr) => { setDropoff(loc); setDropoffAddr(addr); setPickedRouteId(null); }} onPickupPlaceSelect={setPickupPlaceId} onDropoffPlaceSelect={setDropoffPlaceId}>{bookingDetails}</CustomerMapPlanner>)}
  </main></div>;
}
